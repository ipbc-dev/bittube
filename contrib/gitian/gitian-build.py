...
gsigs = 'https://github.com/ipbc-dev/gitian.sigs.git'
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/bittube/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'bittube='+args.commit, '--url', 'bittube='+args.url, infile])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-linux', '--destination', '../sigs/', infile])
        subprocess.check_call('mv build/out/bittube-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/bittube/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/bittube-project/bittube', help='Specify the URL of the repository. Default is %(default)s')
